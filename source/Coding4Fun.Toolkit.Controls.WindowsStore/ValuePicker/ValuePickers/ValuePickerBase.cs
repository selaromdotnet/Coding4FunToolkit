...
{
	[TemplatePart(Name = ButtonPartName, Type = typeof(Windows.UI.Xaml.Controls.Primitives.ButtonBase))]
	public abstract class ValuePickerBase<T> : Control where T : struct
	{
		private const string ButtonPartName = "ValueButton";

		private Windows.UI.Xaml.Controls.Primitives.ButtonBase _valueButtonPart;
		...
		private void OnValueChanged(T? oldValue, T? newValue)
		{
			...
			if (newValue != null && oldValue != null)
				//OnValueChanged(new ValueChangedEventArgs<T>(oldValue, newValue));
				OnValueChanged(new ValueChangedEventArgs<T>(oldValue.Value, newValue.Value));
		}
		...
		protected override void OnApplyTemplate()
		{
			...
			_valueButtonPart = GetTemplateChild(ButtonPartName) as Windows.UI.Xaml.Controls.Primitives.ButtonBase;
		...
		public void OpenPicker()
		{
			...
			if (null == _frame)
			{
				...
				_frame = ApplicationSpace.RootFrame as Frame;
				...
				if (null != _frame)
				{
					...
					_frame.Navigate(typeof(TimeSpanPickerPage));
				}
			}
